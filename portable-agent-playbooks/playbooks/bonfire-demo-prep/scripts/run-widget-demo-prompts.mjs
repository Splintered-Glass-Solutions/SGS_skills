...
async function run() {
	...
	const anonVisitorId = `agent-demo-${Date.now()}`;
	const pageUrl = suite.pageUrl || `${baseUrl}/agent-demo-check`;
	const pageTitle = suite.pageTitle || "the agent demo check";
	...
	const start = await postJson({
		...
		body: {
			...
			metadata: {
				...(suite.metadata || {}),
				page_url: pageUrl,
				source: suite.metadata?.source || "agent-demo-qa",
			},
		},
	});
	const sessionId =
