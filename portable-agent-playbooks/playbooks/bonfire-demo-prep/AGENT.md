# Bonfire Demo Prep Agent Playbook

This is a platform-neutral version of the `bonfire-demo-prep` skill. It is designed
for use with Claude, Claude Code, Cursor, OpenAI agents, or another agent
platform that supports reusable instructions.

## Trigger

Prepare Bonfire demos for a specific customer or organization. Use when auditing Bonfire org/AI/widget setup, testing demo prompts, improving guidance or source-backed behavior, fixing app or test blockers, deploying changes to dev, running Bonfire QA/full-suite checks, and producing a verified demo agenda or talk track.
...
- App/UI/proxy behavior: `<workspace>/bonfire`
- ETL/content/reprocessing: `<workspace>/Bonfire_ETL`
- AI/retrieval/session audits: `<workspace>/Bonfire_AI`

Use these skills when applicable:

- `bonfire-content-etl` or `bonfire-etl-validation` for content/reprocessing questions.
- `bonfire-qaqc`, `full-suite-tests`, and `agent-safe-run` for validation and safe execution.
...
6. **Deploy to dev**
   ...
   - Create a `agent/...` branch from `origin/dev`.
...
Use `scripts/run-widget-demo-prompts.mjs` when the demo uses the public widget/chat path. It bootstraps the widget, starts a session, sends each prompt with `stream: false`, scores the answer, prints JSON, and exits nonzero when any prompt fails. Every logical POST includes a fresh `Idempotency-Key`; if retry support is added, reuse that key for retries of the same logical request. A runner contract failure is not evidence of a browser-client bug.

Example:

```bash
node <agent-config>/skills/bonfire-demo-prep/scripts/run-widget-demo-prompts.mjs \
...
```json
{
  ...
  "pageTitle": "the agent demo check",
  "metadata": { "source": "agent-demo-qa" },
  "prompts": [
    {
      "key": "identity",
