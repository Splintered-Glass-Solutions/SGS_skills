...
Run a proof-based lifecycle sweep across all open agent threads for one project.
The sweep reviews actual work, separates finished work from merely stopped work,
continues safe incomplete work, stages decisions for the user, and closes only
...
1. `<agent-config>/skills/orchestrator-mode/SKILL.md`
2. `<agent-config>/skills/project-portfolio-manager/SKILL.md`
3. `<agent-config>/skills/pm-project-agent/SKILL.md`
4. `<agent-config>/skills/pm-ingest-closeout/SKILL.md`
5. `<agent-config>/skills/close-thread/SKILL.md`
6. `<agent-config>/skills/autonomous-feature-build/SKILL.md`
7. `<agent-config>/skills/feature-finish-line/SKILL.md`
8. `<agent-config>/portfolio/project-registry.md`
9. `<agent-config>/portfolio/thread-registry.md`
10. `<agent-config>/portfolio/approval-ledger.md`
11. `<agent-config>/portfolio/work-ledger.jsonl`
12. `<agent-config>/portfolio/standards-registry.md`
...
clear enough for end-to-end execution without a the user product decision.

### `DECISION_NEEDED`

Progress depends on a real the user choice: product behavior, scope, priority,
...
credentials, rate limits, or another external state that the user cannot resolve
...
the user explicitly decommissions the lane. Review their health and unresolved
...
- no pending the user decision, external send, human acknowledgment, or action;
...
`<agent-config>/portfolio/reports/thread-sweeps/YYYY-MM-DD-HHMM-<project-slug>.md`
