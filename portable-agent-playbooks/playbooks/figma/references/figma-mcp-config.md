...
Use this snippet to register the Figma MCP server in `~/.agent/config.toml` as a streamable HTTP server with bearer auth pulled from your env.
...
- Verify before launching the agent: `echo $FIGMA_OAUTH_TOKEN` should print a non-empty token.

## Setup + verification checklist
- Add the snippet above to `~/.agent/config.toml` under `[mcp_servers.figma]`, and enable `[features].rmcp_client = true` (or `experimental_use_rmcp_client = true` on older releases).
- Restart the agent (CLI/IDE) after updating config and env vars.
