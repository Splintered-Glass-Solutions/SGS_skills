...
- Stage/prod sync and promotion health code: `<workspace>/stage-migration`
- StrIQ backend analytics code and env mapping: `<workspace>/striq-backend`
...
Use `rg` first when locating code. Do not change DB permissions, grants, ownership, or RLS. Do not mutate prod unless the user explicitly asks for a fix, rerun, or deploy; a review should be read-only.
...
4. Check pipeline/job health.
   ...
   - Confirm `ProductionVersions` is on intended analytics versions; do not leave a temporary rollback in place unless the user explicitly wants it.
...
Run these from `<workspace>/stage-migration` unless noted.
...
```bash
node - <<'NODE'
require('dotenv').config({ path: '<workspace>/striq-backend/.env' });
