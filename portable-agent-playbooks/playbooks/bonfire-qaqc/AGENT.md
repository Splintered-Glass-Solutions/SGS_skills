...
Run, resume, reconcile, or report Bonfire QA/QC across frontend pages, interfaces, Web and AI APIs, MCP tools, ETL functions/schedules, and Super Admin. Use for full-suite QA, scoped or delta retests, production readiness, environment validation, blocked-check cleanup, dashboard sessions, manual walkthroughs, release regression checks, and weekly QA reporting. Persist Bonfire runs through the live Supabase QA infrastructure with reviewer Codey and use the Super Admin QA/QC dashboard as the staff interface.
...
the user has approved QA catalog/session/result writes there, but this does not
...
the user Personal, a second-tenant denial fixture, or a provider sandbox.
...
the user has approved contained create, update, and destructive QA actions only
...
Use the user Personal only for checks that explicitly require its MCP binding.
...
features. `QA_SUPER_ADMIN` is `the user@heybonfire.com`; use that approved
...
sign-in blocker and ask the user to refresh that session; do not substitute a
...
for authenticated hosted QA unless the user explicitly requests Chrome.

Production QA is read-only unless the user explicitly approves an exact action.
...
large output through the agent.
