# Clickup Environment Map Agent Playbook

This is a platform-neutral version of the `clickup-environment-map` skill. It is designed
for use with Claude, Claude Code, Cursor, OpenAI agents, or another agent
platform that supports reusable instructions.

## Trigger

Use when working with the user's ClickUp environments, finding ClickUp workspaces/spaces/folders/lists/shared hierarchy, or reading/writing ClickUp tasks, comments, channels, messages, and replies across SGS Consulting, And Studio, SGS_x_S.IS, or Matt's Workspace.
...
Use this skill before ClickUp work where the destination or source could live in the user's SGS workspace, Eric/And Studio, SGS_x_S.IS, Matt's Workspace, a private shared folder/list, or ClickUp Chat. Pair it with the general `clickup` skill for task creation/update conventions.
...
- `<workspace>/sgs_admin/src/sync/load/.env`
- `<workspace>/sgs_admin/src/sync/timeentry/.env`
...
```bash
python3 <agent-config>/skills/clickup-environment-map/scripts/refresh_clickup_map.py
```

As of 2026-06-05, the token authenticated as the user Pope and reached:
