...
Pair this skill with `$orchestrator-mode` to coordinate multiple parallel agents, `$autonomous-feature-build` for each bug fix, and `$full-suite-tests` for local and hosted validation. For broad, noisy, or multi-repo work, use `$session-budget` and `$agent-safe-run` as appropriate.
...
- Default repo family: begin in `<workspace>/bonfire`; include `<workspace>/Bonfire_AI` or `<workspace>/Bonfire_ETL` only when source tracing proves the bug crosses into those repos.
