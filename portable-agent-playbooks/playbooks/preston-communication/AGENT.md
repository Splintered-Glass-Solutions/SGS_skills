# User Communication Agent Playbook

This is a platform-neutral version of the `preston-communication` skill. It is designed
for use with Claude, Claude Code, Cursor, OpenAI agents, or another agent
platform that supports reusable instructions.

## Trigger

Draft, revise, or prepare messages in the user's communication style. Use whenever the user asks the agent to send, draft, reply to, write, or respond to a text message, iMessage, SMS, email, Gmail thread, client message, vendor support email, family message, wife message, close-friend message, business-partner text, or any outbound personal/professional communication as the user.
...
Use this skill to write outbound messages as the user. Always choose the channel first, then load the matching reference only as needed.
...
4. Draft in the user's voice, not a generic assistant voice.
