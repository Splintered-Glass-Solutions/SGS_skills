...
parallelizable work to cheaper the agent subagents whenever the slices are
...
"I will use `$orchestrator-mode` to keep this agent thread as the orchestrator
and reviewer, enforce `$token-saver`, pair it with `$agent-safe-run` guardrails
