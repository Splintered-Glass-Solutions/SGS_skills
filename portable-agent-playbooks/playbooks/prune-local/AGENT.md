# Prune Local Agent Playbook

This is a platform-neutral version of the `prune-local` skill. It is designed
for use with Claude, Claude Code, Cursor, OpenAI agents, or another agent
platform that supports reusable instructions.

## Trigger

Audit and safely prune local Git worktrees and branches across one or more related repositories. Use when the user says prune local, prune everything, clean up old worktrees, or asks which branches still need a bulk-edit collapse. Verify each branch against its repository's local dev target, report unresolved work, cross-repo gaps, and collapse recommendations, then remove only fully integrated and safely disposable local leftovers.
