...
A request to use this skill is approval to inspect local git state, consolidate work locally, resolve conflicts, and run local validation. Pushing a branch or deploying to dev requires the user to explicitly ask for dev deployment in the current conversation.
...
4. Create a consolidation branch.
   - Create a consolidation branch from the correct dev base, such as `agent/dev-consolidation-YYYYMMDD`.
