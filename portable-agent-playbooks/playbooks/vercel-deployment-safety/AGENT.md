...
2. Require the requester to state the client, application, environment, and
   ...
   `<agent-config>/skills/deploy/scripts/resolve_environment.py`; state
...
3. Run the read-only verifier before any write action:
   ...
   ```bash
   python3 <agent-config>/skills/vercel-deployment-safety/scripts/verify_vercel_target.py verify \
...
For STRIQ, also read `<workspace>/striq-shared-docs/VERCEL_DEPLOYMENT_MAP.md`; the registry carries the preflight subset and the shared document records evidence and known local-link hazards.
