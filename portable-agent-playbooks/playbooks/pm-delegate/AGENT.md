# PM Delegate Agent Playbook

This is a platform-neutral version of the `pm-delegate` skill. It is designed
for use with Claude, Claude Code, Cursor, OpenAI agents, or another agent
platform that supports reusable instructions.

## Trigger

PM delegate: delegate highlighted or selected portfolio work to the correct the agent project agent, bounded worker, or approval queue. Use from the project-management thread when the user highlights a blocker, next action, task packet, or PM update line and asks to delegate it.
...
as the delegation candidate. If there is no clear candidate, ask the user for the
...
1. Read `<agent-config>/portfolio/project-registry.md`.
2. Read `<agent-config>/portfolio/thread-registry.md`.
3. Identify the relevant project and any persistent project thread. Treat the
   ...
   available.
4. Read `<agent-config>/portfolio/approval-ledger.md`.
5. Read `<agent-config>/portfolio/work-ledger.md` and
   `<agent-config>/portfolio/work-ledger.jsonl`.
6. Read `<agent-config>/portfolio/pm-ledger-clickup-model.md`.
7. Read `<agent-config>/portfolio/standards-registry.md`.
8. Read `<agent-config>/skills/pm-project-agent/SKILL.md` before
   messaging any persistent Project Agent thread.
9. For broad or multi-project selections, read
   `<agent-config>/skills/orchestrator-mode/SKILL.md` and make an
...
- `persistent_project_thread`: use when the thread registry has a verified
  ...
  or the user explicitly asks to continue/use that existing thread.
...
- `ask_the user`: use when the target project is ambiguous, the selected text
...
continuity, fan-in/fan-out coordination, closeout review, or when the user
...
- Do not send delegated execution into an old project thread merely because it
  ...
  project continuity/fan-in or the user explicitly asks to use that thread.
...
7. Create ClickUp only when the delegation produces actionable follow-up that
   the user or another worker must act on and task creation is authorized.
...
3. Send a bounded prompt only when current authority permits internal the agent
...
- Broad Bonfire project-agent/orchestrator context should prefer
  `<workspace>/bonfire_shared_docs`; scoped app QA and implementation
  should use `<workspace>/bonfire`.
- Broad StrIQ project-agent/orchestrator context should prefer
  `<workspace>/striq-shared-docs`; scoped frontend/backend/stage work
...
- Personal ops, client communications, and support/customer-facing work must not
  send external messages unless the user explicitly asks to send.
