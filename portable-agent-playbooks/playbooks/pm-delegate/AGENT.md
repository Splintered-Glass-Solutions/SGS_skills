...
3. Identify the relevant project and its persistent project thread. If the
   ...
   `ask_the user`, `action_proposal`, or `no_action`.
...
6. Read `<agent-config>/portfolio/standards-registry.md`.
...
- Do not silently spawn a bounded worker directly from the portfolio manager
  ...
  messaging tooling is unavailable, or the user explicitly asks the manager
...
3. Send the packet to the relevant project thread when thread messaging tooling
   is available and authority permits internal agent thread messaging. The
...
4. Spawn a bounded worker directly from the PM thread only when no verified
   ...
   the user explicitly asks for direct PM-thread spawning. Include the reason in
