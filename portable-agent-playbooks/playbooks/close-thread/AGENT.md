...
Close the current agent thread by unpinning it when needed and renaming it with a leading fullwidth dash marker. Preserve the reserved `🗄️` marker when closing a sidelined feature task. Use when the user asks to close out, mark closed, or run the close-thread slash command for the current thread.
