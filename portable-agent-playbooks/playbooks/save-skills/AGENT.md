...
reusable agent skills and slash commands. This is a pattern-mining and local
...
Do not push to Git from this skill unless the user explicitly also requests
...
1. Read and follow:
   - `<agent-config>/skills/.system/skill-creator/SKILL.md`
   - `<agent-config>/skills/agent-slash-command-skill/SKILL.md`
2. Inventory existing user-level skills and slash wrappers:

```bash
rg --files <agent-config>/skills -g 'SKILL.md'
rg --files <agent-config>/skills -g 'agents/openai.yaml'
rg --files <agent-config>/commands -g '*.md'
```
...
- a workflow the user explicitly says should become reusable.
...
- recurrence is at least 2, unless the user explicitly requested the skill;
...
as deferred so the user can review them without expanding the picker too quickly.
...
2. Create or update `<agent-config>/skills/<name>/SKILL.md`.
3. Create or update `<agent-config>/skills/<name>/agents/openai.yaml`.
4. Create `<agent-config>/commands/<name>.md` because this workflow is
...
- Do not update memory, standards registries, or project registries unless
  the user separately asks for those changes.
...
```bash
python3 <agent-config>/skills/.system/skill-creator/scripts/quick_validate.py <agent-config>/skills/<name>
```
