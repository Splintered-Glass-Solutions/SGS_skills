...
Do not generate duplicate marketing assets. Before creating a new package, run `/Users/preston/.codex/skills/marketing-asset-generation/scripts/find_marketing_asset_package.py --feature-name "<Feature Name>"` from the relevant repo. Reuse the existing package when found. Only run `marketing-asset-generation` when no package exists or Preston explicitly asks for fresh assets.
...
10. Prepare release notes, social copy, and marketing assets.
   ...
   - First run `/Users/preston/.codex/skills/marketing-asset-generation/scripts/find_marketing_asset_package.py --feature-name "<Feature Name>"` from the relevant repo. If it finds an `asset-manifest.json`, reuse that package and include its path in the handoff.
