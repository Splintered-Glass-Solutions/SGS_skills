---
name: tiburon-feature-release-update
description: Create and send Caleb concise Tiburon post-feature release updates after implementation, validation, migration, or deployment work. Use when Preston asks to update Caleb, send Tiburon release notes, report what shipped, explain a completed feature, share testing instructions or screenshots, request user testing, or summarize Tiburon release status and next steps.
---
...
invoked. Draft only when Preston explicitly asks to review first, draft only, or
not send.
...
- Use `deploy` when Preston asks to publish changes before the update.
- Use `full-suite-tests` when the update requires broad test proof.
- Use `preston-communication` to match Preston's normal email voice.
...
6. Send unless Preston requested draft-only.
...
- Do not attach raw tickets unless Preston explicitly approves that exact file.
