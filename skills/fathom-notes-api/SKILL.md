...
- If missing, source `/Users/preston/.codex/secrets/fathom.env` if it exists.
- Never print, echo, log, or paste the API key.
- Do not use `FATHOM_WEBHOOK_SECRET` for reads. It is only for webhook verification.
- If a webhook secret or API key is pasted into chat or output, tell Preston to rotate it if the thread is not private.
...
3. Probe identity if needed:
   ...
   - These may return empty for Preston's key. If so, infer Preston from successful `recorded_by[]=preston@splinteredglass.solutions` results.
4. List Preston-recorded meetings:
   - `GET /meetings?include_summary=true&include_action_items=true&created_after=...&created_before=...&recorded_by[]=preston@splinteredglass.solutions`
...
```sh
node /Users/preston/.codex/skills/fathom-notes-api/scripts/fathom_api.mjs meetings \
  --start 2026-06-02T05:00:00Z \
  --end 2026-06-08T20:31:52Z \
  --recorded-by preston@splinteredglass.solutions \
  --out /tmp/fathom/meetings.json
```

```sh
node /Users/preston/.codex/skills/fathom-notes-api/scripts/fathom_api.mjs transcripts \
  --ids 152238927,152444689,153196836 \
  --out-dir /tmp/fathom
```

The script loads `/Users/preston/.codex/secrets/fathom.env` automatically if `FATHOM_API_KEY` is not already set.
