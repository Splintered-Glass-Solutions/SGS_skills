---
name: automation-thread-scheduler
description: >-
  Create or repair Codex automations and scheduled monitors with thread hygiene:
...
Use this skill whenever creating or maintaining a Codex automation or scheduled
monitor. The default is a visible, same-thread automation that does not create a
new Codex thread every run.
...
- The automation prompt must explicitly say to continue the existing Codex
...
```text
Thread visibility:
- Continue this existing Codex thread for every run.
- Do not create, fork, hand off, or open a new Codex thread for this monitor.
