---
name: prep-collapse-bulk-edits
description: >-
  Prepare the current Codex thread for safe collapse of bulk edits by auditing
  every requested change, finding unfinished work and broken fixes, repairing
  authorized feature defects, rerunning focused validation, completing the
  finish-line evidence, and producing a collapse-ready handoff. Use before
  $bulk-edits-thread or $collapse-bulk-edits-thread when a thread contains
  several edits, tests, migrations, branches, or worktrees that must be brought
  to one verified state.
---
...
- `$test` for feature-scoped deterministic, integration, local-stack, and
  browser evidence.
- `$codex-safe-run` for one-shot/file-backed logs and process discipline.
