...
- Verify actual logo pixels and the saved brand palette in the generated PDF, both inline and downloaded. A brand name, colored heading, assistant success message, or working logo in Settings does not prove the PDF is branded. Treat broken or missing marks as recording blockers. Inspect every printed page for split lists, isolated footer pages, and low-contrast headings. Check the actual download/export path with its organization branding, not a lower-level renderer invoked without those settings. For short handouts, prefer a readable single-page layout and verify that tightening it preserves the wording.
...
- For incoming-email demos, record send intent before the provider call so uncertain sends are inspected rather than retried blindly. Verify real inbox arrival, original-thread DRAFT state, a nonmatching no-draft branch, a missing-information reply, and duplicate protection. Restore temporary allowances and pause test processing; preserve review drafts unsent.
