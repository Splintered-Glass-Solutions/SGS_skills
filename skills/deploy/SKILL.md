...
name: deploy
description: >-
  Deploy the feature, functionality, branch, commit, or PR worked on in the
  current Codex task to an explicit environment or the sole registered
  environment for a single-target project, including dependency-aware
  deployment across multiple repositories such as frontend, backend, API,
  workers, infrastructure, and database components. Recover or create missing
  deployable artifacts when the current task contains a decision-ready feature
  scope. Use when Preston invokes $deploy or /deploy, including a bare "deploy"
  in a registered single-environment project, and expects Codex to persist until
  every in-scope component is deployed or a genuine external blocker is proven.
---
...
```bash
python3 /Users/preston/.codex/skills/deploy/scripts/resolve_environment.py resolve \
...
`/Users/preston/.codex/skills/vercel-deployment-safety/references/project-registry.json`;
...
5. Keep working through safe, in-scope steps while the next action remains under
   Codex control. Do not stop merely because the rollout is slow, cross-repo,
...
Use `orchestrator-mode` and `codex-safe-run` for multi-repo, multi-provider,
...
- `DEPLOYED — COMPLETE`: every graph node has verified target-environment
  evidence.
- `DEPLOYMENT IN PROGRESS`: provider work is still non-terminal and Codex is
