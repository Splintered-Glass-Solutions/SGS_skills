...
parallelizable work to cheaper Codex subagents or parallel tool waves only when
...
7. Use `$codex-safe-run` when the task may create many tool calls, long logs,
...
"I will use the current model in this Codex thread as the orchestrator and
reviewer, use `$codex-safe-run` guardrails for long or noisy work, and use
