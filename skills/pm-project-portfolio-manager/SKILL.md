...
Read the approval ledger before reporting items that are waiting on Preston.
When a run creates, clears, supersedes, or confirms a Preston-gated decision,
...
9. Ingest only relevant current state:
   ...
   - existing Codex thread state when continuing known work
10. Classify each signal:
   ...
   - `ask_preston`
   - `escalate`
11. For each `ask_preston` or gated `escalate` signal, reconcile it against the
   approval ledger:
...
17. Fan in worker closeouts, spot-check important proof, and update the relevant
   ...
   work item or Preston-gated item changed state.
18. At the end of scheduled dispatcher runs, append a dispatcher scorecard entry
   ...
   blocker, resolved waiting work, or removed a Preston decision loop.
